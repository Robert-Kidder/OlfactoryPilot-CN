...
def _build_flow_context(low_flow_threshold: float = 0.2):
    ...
    controller = MainController(
        state,
        worker,
        safety_manager=SafetyManager(low_flow_threshold=low_flow_threshold),
        allow_test_actuation_bridge=True,
    )
    original_handle_telemetry = controller.handle_telemetry

    def feed_production_telemetry(payload):
        worker._connected = bool(payload.get("connected", worker._connected))
        worker.consume_airflow_sample(
            float(payload.get("airflow", 0.0)),
            float(payload.get("timestamp", time.time())),
            None,
        )
        return original_handle_telemetry(payload)

    controller.handle_telemetry = feed_production_telemetry
...
def test_successful_manual_plan_result_refreshes_valve_ui_from_committed_state(qt_app):
    _, controller, window, _ = _build_flow_context()
    request_id = "manual-refresh"
    controller._pending_plan_ui[request_id] = {"kind": "manual", "channels": [1]}
    with controller.valve_service._state_lock:
        controller.valve_service._states[1] = True
        controller.valve_service._states["master"] = True
    assert window.pretest_view._open_states[1] is False

    controller._handle_actuation_plan_result(
        {"request_id": request_id, "success": True, "message": "ok"}
    )

    assert window.pretest_view._open_states[1] is True
...
def test_apply_flow_does_not_switch_master_valve(qt_app):
    ...
    controller.handle_telemetry({"airflow": 0.0, "connected": True, "timestamp": 1.0})
    safety_events = list(hal.master_events)

    result = controller.handle_apply_request(120.0, 800.0, 300.0)

    assert result.success is True
    assert hal.master_events == safety_events
...
def test_valve_click_only_stages_channel_until_start(qt_app):
    state, controller, window, hal = _build_flow_context(low_flow_threshold=0.1)
    controller.handle_telemetry({"airflow": 1.0, "connected": True, "timestamp": 1.0})
    staged_state = hal.get_line_state("Dev1/P0.0")
    safety_events = list(hal.master_events)

    window.pretest_view._handle_click(1)

    assert window.pretest_view.selected_valves() == [1]
    assert hal.get_line_state("Dev1/P0.0") is staged_state
    assert hal.master_events == safety_events
