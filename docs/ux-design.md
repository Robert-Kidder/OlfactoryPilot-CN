...
- `InfoBar` 只提示新异常或用户需要知道的操作结果；同态提示不重复弹出。普通连接成功和无操作价值的正常阶段不创建提示。只有真正阻断当前操作的问题才使用 Fluent Dialog，不使用 `QMessageBox`。

其他能力的页面结构不在当前权威 UX 中预设；立项时应从用户任务与已验证领域契约重新设计。清洗、配置等既有服务层安全约束仍以 PRD 与架构为准。
