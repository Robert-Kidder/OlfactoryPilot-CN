...
- 主窗口显示后固定自动尝试连接一次。连接过程中 Header 只显示“正在连接…”，完整成功后原位显示“设备已连接”，不弹连接成功 InfoBar；正常启动不显示主“连接设备”按钮，也不提供 auto-connect 设置。
- 当前没有连接时统一显示“设备未连接”和“重新连接”。这包括启动连接失败、Global Stop 安全收口成功及运行中断线安全收口成功；三种情况均不自动重复连接，只有用户点击“重新连接”才调用同一连接事务。
- 普通界面不显示自动连接次数、内部连接阶段、Global Stop 完成说明或断线分类，也不弹“已安全停止”“设备已断开”等重复通知。技术原因只写日志；只有无法确认阀门关闭、流量归零或 SafeStop 完成等需要用户立即处理的情况，才持续显示“设备未能正常停止，请立即关闭设备电源”。
- Header 的状态区和动作槽保持固定宽度；“正在连接…”、“设备已连接”、“设备未连接”及“重新连接”按钮的显隐不得引起几何跳动。窗口在 queued auto-connect 前关闭时不产生后台连接。
...
- simulation 中 Global Stop 或安全收口后的表现同样为“设备未连接”和“重新连接”；等待不会自动重连，人工重新连接成功后也不会恢复旧动作。
